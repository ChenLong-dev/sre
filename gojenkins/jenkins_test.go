...
var TestCookieData = map[string]string{
	"url":             "localhost",
	"authUser":        "admin",
	"authPwd":         "admin",
	"jUser":           "admin",
	"jPwd":            "admin",
	"identifyContent": "content",
}

func TestInit(t *testing.T) {
	jenkins = CreateJenkins(nil, &Config{
...
func TestCreateJobs(t *testing.T) {
	...
	t.Run("with-cookie", func(t *testing.T) {
		jenkins := CreateJenkins(nil, &Config{
			BaseURL: TestCookieData["url"],
			BasicAuth: &BasicAuthConfig{
				UserName: TestCookieData["authUser"],
				Password: TestCookieData["authPwd"],
			},
			Crowd: &CrowdConfig{
				UserName:        TestCookieData["jUser"],
				Password:        TestCookieData["jPwd"],
				IdentifyContent: TestCookieData["identifyContent"],
			},
		})
		...
		job_data := getFileAsString("job.xml")
		job, err := jenkins.CreateJob(job_data, "job1")
		assert.Nil(t, err)
		assert.NotNil(t, job)
	})

	t.Run("with-cookie-job-exist", func(t *testing.T) {
		jenkins := CreateJenkins(nil, &Config{
			BaseURL: TestCookieData["url"],
			BasicAuth: &BasicAuthConfig{
				UserName: TestCookieData["authUser"],
		...
		job_data := getFileAsString("job.xml")
		_, err = jenkins.CreateJob(job_data, "job1")
		assert.NotNil(t, err)
	})
}
...
func TestCreateNodes(t *testing.T) {
	...
	node1, err := jenkins.CreateNode(id1, 1, "Node 1 Description", "/Users/mac/jenkins/demo4", "", jnlp)
	assert.NoError(t, err)
	assert.Equal(t, id1, node1.GetName())

	node3, _ := jenkins.CreateNode(id3, 1, "Node 3 Description", "/Users/mac/jenkins/demo4", "jdk7")
	assert.Equal(t, id3, node3.GetName())
	node4, _ := jenkins.CreateNode(id4, 1, "Node 4 Description", "/Users/mac/jenkins/demo4", "jdk7")
...
func TestGetSingleJob(t *testing.T) {
	...
	t.Run("with-cookie-retry", func(t *testing.T) {
		jenkins := CreateJenkins(nil, &Config{
			BaseURL: TestCookieData["url"],
			BasicAuth: &BasicAuthConfig{
				UserName: TestCookieData["authUser"],
		...
		_, err := jenkins.Init()
		assert.Nil(t, err, "Jenkins Initialization should not fail")
		_, err = jenkins.GetJob("job1")
		assert.NoError(t, err)
	})
	t.Run("with-cookie-not-exist", func(t *testing.T) {
		jenkins := CreateJenkins(nil, &Config{
			BaseURL: TestCookieData["url"],
			BasicAuth: &BasicAuthConfig{
				UserName: TestCookieData["authUser"],
		...
		_, err := jenkins.Init()
		assert.Nil(t, err, "Jenkins Initialization should not fail")
		_, err = jenkins.GetJob("job-not-exit")
		assert.NotNil(t, err)
	})
}
