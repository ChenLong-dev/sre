...
type CrowdConfig struct {
	UserName        string `json:"user_name" yaml:"userName"`
	Password        string `json:"password" yaml:"password"`
	CheckPath       string `json:"check_path" yaml:"checkPath"`
	IdentifyContent string `json:"identify_content" yaml:"identifyContent"`
}

type Config struct {
	BaseURL   string           `json:"base_url" yaml:"baseURL"`
	BasicAuth *BasicAuthConfig `json:"basic_auth" yaml:"basicAuth"`
	Crowd     *CrowdConfig     `json:"crowd" yaml:"crowd"`
}
...
func CreateJenkins(client *http.Client, c *Config) *Jenkins {
	j := &Jenkins{}
	if strings.HasSuffix(c.BaseURL, "/") {
		c.BaseURL = c.BaseURL[:len(c.BaseURL)-1]
	}
	j.Server = c.BaseURL
	j.Requester = &Requester{Config: c, SslVerify: true, Client: client}
